	...
	for ($i=1;$i<=$txtContador;$i++){
		for($j=0;$j<count($txtIdIconos);$j++){
			...
			if($ExistePer==true){
				...
				$ActualizaPermisos = $ObjetosPermisos->AsignaPermisosRoles($IdPerfil,$IdIcono,$IdMenu,$Estatus,"1");
			}else{
				//Si no existe insertamos  nomas las que esten activas
				if($Estatus==1){
					$ActualizaPermisos = $ObjetosPermisos->AsignaPermisosRoles($IdPerfil,$IdIcono,$IdMenu,$Estatus,"0");
				}
			}
		}
	}
